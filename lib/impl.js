	...
	//funcion que dice la palabra pasada como parametro
		...
		descargarMapasPrivada = function(){
			...
			if(peer == null || errores){
				peer = new Peer('celda_app_peer_nico_1', {key: 'ino3l998li60f6r', debug: 3});			
			}
			//se conecta con el id del otro peer
			connection = peer.connect('celda_map_editor_peer_nico_1');		  	
			...
			connection.on('open', function(){
				connection.send('Hola, pasa el mapa, wacho. Argentina campeon!');						
			});
			...
			connection.on('data', function(data) {
				...
				if(data != null){
					...
					data = JSON.parse(data);
                                        ...
                                        var cant_celdas_alto = data[0];
                                        var cant_celdas_largo = data[1];
                                        var cant_celdas_orientacion_inicio = data[2];
                                        var celda_largada = data[3];
                                        var celda_llegada = data[4];
                                        var camino = data[5];
                                        var array_obstaculos = data[6];
